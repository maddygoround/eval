...
from src.eval_framework.core.evaluator import ResponseEvaluator
from src.eval_framework.core.judge import PetriJudge
...
class TestConfidenceCalibration:
    ...
    async def test_overconfidence_detection(self):
        ...
        overconfident = await evaluator.check_confidence_calibration(response)

        assert len(overconfident) > 0, "Should detect overconfident language"

    @pytest.mark.asyncio
    async def test_appropriate_confidence(self):
...
class TestComprehensiveEvaluation:
    ...
    async def test_bad_response(self):
        ...
        assert result["overall_score"] < 0.7, "Bad response should score poorly"
